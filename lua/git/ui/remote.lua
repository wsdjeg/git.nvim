...
local function on_stderr(_, _) end
...
local function update_buf_context()
  ...
  util.update_buffer(bufnr, context)
end
...
local function on_exit(_, code, signal)
  ...
  if #update_branch_remote_list > 0 then
    log.debug(
      'update_branch_remote_list is:'
        .. vim.inspect(update_branch_remote_list)
    )
...
local function get_cursor_info()
  ...
  elseif vim.startswith(l, '        ') then
    ...
    if remote_line > 0 then
      c.branch = string.gsub(
        string.sub(vim.fn.getline(remote_line), 7),
        updating_extra_text,
        ''
      ) .. '/' .. string.sub(l, 12)
    end
...
function M.update_branch_list(name)
  if update_branch_list_jobid ~= -1 then
    ...
    if
      name ~= update_branch_list_name
      and not is_in_list(update_branch_remote_list, name)
    then
      -- 此处应该检查list里是否包含name
  ...
  update_branch_list_jobid = job.start(
